        ...
        <div className="border-l border-dashed border-slate-200 dark:border-slate-800 pl-4 py-1.5 relative">
            ...
            >
                <div className="flex items-center gap-1.5 font-bold">
                    ...
                    <span>
                        {isExpanded ? "Hide" : "Show"} {collapsedEvents.length}{" "}
                        other standard trace{" "}
                        {collapsedEvents.length === 1 ? "event" : "events"}
                    </span>
                    <span className="text-[9.5px] font-bold text-slate-600 dark:text-slate-350 italic">
                        (
                        {collapsedEvents
                            .map((e) => e.sessionEvent.type.split(".").pop())
                            .join(", ")}
                        )
                    </span>
                </div>
            ...
            {isExpanded && (
                <div className="mt-1.5 flex flex-col gap-2 pl-2 overflow-hidden">
                            ...
                            >
                                <div className="flex items-center justify-between gap-2">
                                    ...
                                    <span className="text-[9.5px] font-mono text-slate-605 dark:text-slate-350 font-semibold">
                                        {new Date(
                                            subEvent.sessionEvent.timestamp,
                                        ).toLocaleTimeString([], {
                                            hour: "2-digit",
                                            minute: "2-digit",
                                            second: "2-digit",
