...
interface ActionHistoryGroupProps {
    ...
    readonly segmentActionHistory: (
        events: readonly CopilotEvent[],
    ) => readonly SegmentedNode[];
    ...
    readonly cardInnerTabs: Record<
        string,
        "details" | "json" | "stream" | undefined
    >;
...
export function ActionHistoryGroup({
    ...
    const toolCount = events.filter((e) =>
        e.sessionEvent.type.startsWith("tool."),
    ).length;
    const thoughtCount = events.filter(
        (e) =>
            e.sessionEvent.type === "assistant.reasoning" ||
            e.sessionEvent.type.includes("reasoning"),
    ).length;
    ...
    const durationStr = React.useMemo(() => {
        ...
        const lastTime = new Date(
            events[events.length - 1]?.sessionEvent.timestamp || "",
        );
    ...
    return (
        <div className="relative flex flex-col items-start w-full text-left my-2">
            <div
                ...
                {isExpanded && (
                    ...
                    >
                        <div className="flex flex-col gap-1 pl-4 ml-2 border-l-2 border-zinc-100 dark:border-zinc-800/60">
                            {segmentActionHistory(events).map(
                                (subNode, subIdx) => {
                                    if (
                                        subNode.type === "collapsed" &&
                                        subNode.events
                                    ) {
                                        return (
                                            <div key={subNode.id}>
