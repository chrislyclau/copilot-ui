...
export function EventCard({
    ...
    const binaryItems = pBinaryResults as Array<{
        mimeType?: string;
        description?: string;
        data?: string;
    }>;
    ...
    if (event.category === "user") {
        avatarBg =
            "bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-350 border border-zinc-250 dark:border-zinc-700 shadow-2xs";
        nodeBorder = isFocused
            ? "border-zinc-400 dark:border-zinc-600"
            : "hover:border-zinc-200 dark:hover:border-zinc-800";
        badgeStyle =
            "bg-zinc-50 dark:bg-zinc-900/40 text-zinc-700 dark:text-zinc-350 border border-zinc-250 dark:border-zinc-800";
        NodeIcon = User;
    } else if (event.category === "assistant") {
        avatarBg =
            "bg-sky-500/10 text-sky-500 border border-sky-550/20 shadow-xs";
        nodeBorder = isFocused
            ? "border-sky-500/30"
            : "hover:border-sky-500/10";
        badgeStyle =
            "bg-sky-50 dark:bg-sky-950/20 text-sky-600 dark:text-sky-350 border border-sky-100/50 dark:border-sky-900/40";
        NodeIcon = Sparkles;
        ...
        badgeStyle =
            "bg-amber-50 dark:bg-amber-950 text-amber-700 dark:text-amber-400 border border-amber-100 dark:border-amber-900/50";
        NodeIcon = Terminal;
        ...
        badgeStyle =
            "bg-purple-50 dark:bg-purple-950 text-purple-700 dark:text-purple-400 border border-purple-100 dark:border-purple-950/50";
        NodeIcon = Lock;
        ...
        badgeStyle =
            "bg-rose-50 dark:bg-rose-950 text-rose-700 dark:text-rose-400 border border-rose-100 dark:border-rose-900/50";
        NodeIcon = ShieldAlert;
    } else if (event.category === "system") {
        avatarBg =
            "bg-zinc-200 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-350 shadow-2xs";
        nodeBorder = isFocused
            ? "border-zinc-400 dark:border-zinc-700"
            : "hover:border-zinc-300 dark:hover:border-zinc-800";
        badgeStyle =
            "bg-zinc-100 border border-zinc-200 dark:bg-zinc-850 text-zinc-700 dark:text-zinc-300 dark:border-zinc-750";
        NodeIcon = Settings;
    }
    ...
    if (event.sessionEvent.type === "composer.plan_mutated") {
        ...
        badgeStyle =
            "bg-amber-100 dark:bg-amber-950/45 text-amber-800 dark:text-amber-300 border border-amber-300 dark:border-amber-800/60";
        NodeIcon = Sparkles;
    }
    ...
    if (
        event.sessionEvent.type === "turn.start" ||
        event.sessionEvent.type === "TURN_COMPLETED"
    ) {
        NodeIcon = Play;
    ...
    if (
        event.sessionEvent.type === "subtask.start" ||
        event.sessionEvent.type === "subtask.complete"
    ) {
        NodeIcon = ListTodo;
        avatarBg =
    ...
    return (
        ...
        >
            <div
                className={`w-full overflow-hidden transition-all duration-150 group`}
            >
                <div className="px-1 py-1 flex items-start gap-3 select-none">
                    {!isNested && (
                        <div
                            className={`mt-0.5 w-6 h-6 rounded-full flex items-center justify-center shrink-0 ${avatarBg}`}
                        >
                            <NodeIcon
                                size={event.category === "system" ? 12 : 14}
                                className="shrink-0"
                            />
                        </div>
                    )}
                    ...
                    <div className="flex flex-col gap-0.5 w-full">
                        <div className="flex flex-wrap items-center justify-between gap-2 text-xs w-full">
                            <div className="flex flex-wrap items-center gap-2">
                                ...
                                {!isExpanded && (
                                    <div className="hidden md:flex items-center gap-1.5 text-xs text-slate-400 font-mono pr-2 max-w-[200px] truncate opacity-0 group-hover:opacity-100 transition-opacity">
                                        {event.sessionEvent.type ===
                                            "tool.execution_start" &&
                                            `args: ${Object.keys(pArguments || {}).join(", ")}`}
                                        {event.sessionEvent.type ===
                                        ...
                                        {event.sessionEvent.type ===
                                            ...
                                            `"${pThought?.slice(0, 30)}..."`}
                                        {event.sessionEvent.type ===
                                            "session.error" && `err: ${pError}`}
                                        {event.sessionEvent.type ===
                                            "gate.start" &&
                                            `gate: ${event.sessionEvent.data.gateName}`}
                                        {event.sessionEvent.type ===
                                            "gate.result" &&
                                            `pass: ${event.sessionEvent.data.pass ? "true" : "false"}`}
                                    </div>
                                )}
                            </div>
                            ...
                            >
                                <span className="font-mono text-[9px] text-slate-400 dark:text-slate-600 opacity-0 group-hover:opacity-100">
                                    {timestampStr}
                                </span>
                                <button
                                    id={`btn-expand-card-${event.sessionEvent.id}`}
                                    onClick={(e) =>
                                        toggleExpand(event.sessionEvent.id, e)
                                    }
                                ...
                                >
                                    <ChevronDown
                                        size={14}
                                        className={`transform transition-transform duration-200 ${isExpanded ? "rotate-180" : "-rotate-90"}`}
                                    />
                                </button>
                            </div>
                        </div>
                                    ...
                                    <button
                                        ...
                                        onClick={(e) => {
                                            ...
                                            copyToClipboard(
                                                ...
                                                `evt-short-${event.sessionEvent.id}`,
                                            );
                                        }}
                            ...
                            {innerTab === "details" ? (
                                <div className="text-sm">
                                    {event.sessionEvent.type ===
                                        ...
                                        <div className="flex flex-col gap-3 mt-1.5 p-4 bg-amber-50/50 dark:bg-amber-950/10 border-amber-300 dark:border-amber-900 rounded-xl border-2 text-left text-xs shadow-md">
                                            ...
                                            <p className="text-[11px] text-slate-600 dark:text-slate-400">
                                                A persistent pipeline bottleneck
                                                was detected. The runtime
                                                environment has dynamically
                                                mutated the workspace blueprint
                                                to inject/modify mandatory
                                                verification gates and clear the
                                                blocker.
                                            </p>
                                            ...
                                            <div className="space-y-2 pt-2 border-t border-amber-100 dark:border-amber-900/50">
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {(
                                                        ...
                                                        <div
                                                            key={gate}
                                                            className="px-2 py-1 bg-white/90 dark:bg-black/55 border border-amber-200 dark:border-amber-800/80 rounded font-mono text-[10px] text-amber-700 dark:text-amber-400 font-bold shadow-xs"
                                                        >
                                                            {gate}
                                                        </div>
                                                    ))}
                                    ...
                                    {event.sessionEvent.type ===
                                        ...
                                        <div className="flex flex-col gap-3 mt-1.5 p-4 bg-rose-50/50 dark:bg-rose-950/10 border-rose-300 dark:border-rose-900 rounded-xl border-2 text-left text-xs shadow-md">
                                            ...
                                            <p className="text-[11px] text-slate-600 dark:text-slate-400">
                                                ...
                                                halted because the project goal
                                                contains critical ambiguities.
                                                Please address the following
                                                variables to proceed:
                                            </p>

                                            <div className="space-y-2 pt-2 border-t border-rose-100 dark:border-rose-900/50">
                                                <div className="text-[10px] uppercase font-bold text-rose-700 dark:text-rose-400 tracking-wider">
                                                ...
                                                <div className="flex flex-col gap-2">
                                                    {(
                                                        ...
                                                        <div
                                                            key={v}
                                                            className="flex items-start gap-2 p-2 bg-white/90 dark:bg-black/55 border border-rose-250 dark:border-rose-800/80 rounded shadow-xs"
                                                        >
                                                            <input
                                                                type="checkbox"
                                            ...
                                            <div className="mt-2 text-[10px] text-rose-500 font-semibold italic">
                                                Action Required: Refine your
                                                request to resolve these
                                                check-items.
                                            </div>
                                        </div>
                                    )}
                                        ...
                                        <div className="flex flex-col gap-3 mt-1.5 p-4 bg-indigo-50/50 dark:bg-indigo-950/10 border-indigo-200 dark:border-indigo-900 rounded-xl border text-left text-xs">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="font-bold text-indigo-800 dark:text-indigo-300">
                                                    Composer Blueprint:{" "}
                                                    {event.sessionEvent.data.taskType?.toUpperCase() ||
                                                        "GENERAL FEATURE"}
                                                </span>
                                            </div>
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {(
                                                        ...
                                                        <div
                                                            key={gate}
                                                            className="px-2 py-1 bg-white/80 dark:bg-black/40 border border-slate-200 dark:border-slate-800 rounded font-mono text-[10px] text-indigo-600 dark:text-indigo-400 font-bold"
                                                        >
                                                            {gate}
                                                        </div>
                                                    ))}
                                    ...
                                    {event.sessionEvent.type ===
                                        "loop.complete" && (
                                        <div
                                            className={`flex flex-col gap-2 mt-1.5 p-3.5 ${pResultType === "failure" ? "bg-rose-50/50 dark:bg-rose-950/10 border-rose-200 dark:border-rose-900" : "bg-emerald-50/50 dark:bg-emerald-950/10 border-emerald-200 dark:border-emerald-900"} rounded-xl border text-left text-xs`}
                                        >
                                            <div className="flex items-center gap-2">
                                                {pResultType === "failure" ? (
                                                    <XCircle
                                                ...
                                                <span
                                                    className={`font-bold ${pResultType === "failure" ? "text-rose-800 dark:text-rose-400" : "text-emerald-800 dark:text-emerald-400"}`}
                                                >
                                                    {pSummary}
                                                </span>
                                            </div>
                                    ...
                                    {event.sessionEvent.type ===
                                        ...
                                        <div className="flex flex-col gap-2 mt-1.5 p-3.5 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800/80 text-left">
                                            <div className="flex items-center gap-3 text-xs">
                                                <div
                                                    className={`flex items-center gap-1.5 font-bold ${event.sessionEvent.data.pass ? "text-emerald-600" : "text-rose-600"}`}
                                                >
                                                    {event.sessionEvent.data
                                                        .pass ? (
                                                        <CheckCircle2
                                    ...
                                    {event.sessionEvent.type ===
                                        ...
                                        <div className="flex flex-col gap-2 mt-1.5 p-3.5 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800/80 text-left text-xs">
                                            ...
                                            <div className="flex items-center gap-2 font-mono text-[11px] bg-slate-100 dark:bg-slate-950 p-2 rounded-lg border border-slate-200 dark:border-slate-850 mt-1 text-slate-600 dark:text-slate-300">
                                                ...
                                                <span className="bg-slate-200 dark:bg-slate-800 px-1.5 py-0.5 rounded select-all">
                                                    {
                                                        event.sessionEvent.data
                                                            .currentModel
                                                    }
                                                ...
                                                <span className="bg-emerald-50 dark:bg-emerald-950 border border-emerald-250/30 px-1.5 py-0.5 rounded text-emerald-700 dark:text-emerald-400 font-bold select-all">
                                                    {
                                                        event.sessionEvent.data
                                                            .nextModel
                                                    }
                                            ...
                                            {event.sessionEvent.data
                                                ...
                                                <div className="text-[11px] text-slate-550 dark:text-slate-400 pt-0.5">
                                                    Failed Gate:{" "}
                                                    <code className="bg-rose-50 dark:bg-rose-950/20 text-rose-600 dark:text-rose-400 px-1.5 py-0.5 rounded font-mono font-bold text-[10px] border border-rose-100 dark:border-rose-900/40">
                                                        {
                                                            event.sessionEvent
                                                                .data.failedGate
                                                        }
                                    ...
                                    {event.sessionEvent.type ===
                                        ...
                                        <div className="mt-3 p-4 bg-amber-50/50 dark:bg-amber-950/10 border border-amber-250/70 dark:border-amber-900/60 rounded-xl space-y-3 text-left">
                                            ...
                                            <p className="text-[11px] text-slate-650 dark:text-slate-400 leading-normal">
                                                {event.sessionEvent.data
                                                    .summary ||
                                                    "All automated validation retries have been exhausted. Please review the failed gate details and provide corrective directions below to resume the execution pipeline."}
                                            </p>
                                            ...
                                            {event.sessionEvent.data
                                                ...
                                                <p className="text-[11px] text-slate-500 font-mono">
                                                    Blamed Gate:{" "}
                                                    <strong className="text-rose-600 dark:text-rose-400 border border-rose-100 dark:border-rose-950 px-1.5 py-0.5 rounded bg-rose-50/40 dark:bg-rose-950/20">
                                                        {
                                                            event.sessionEvent
                                                                .data.failedGate
                                                        }
                                                ...
                                                {pAttachments &&
                                                    pAttachments.length > 0 && (
                                                        <div className="mt-3 pt-2.5 border-t border-zinc-200 dark:border-zinc-800 flex flex-wrap items-center gap-2">
                                                            <span className="text-[10px] text-zinc-400 dark:text-zinc-500 uppercase font-bold tracking-wider mr-1 font-mono">
                                                                Attachments:
                                                            </span>
                                                            {pAttachments.map(
                                                                (
                                                                    ...
                                                                    return (
                                                                        <div
                                                                            ...
                                                                            className="flex items-center gap-1 bg-zinc-200/60 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-350 rounded px-2 py-0.5 text-xs font-mono"
                                                                        >
                                                                            <FolderOpen
                                                                                size={
                                                                                    11
                                    ...
                                    {event.sessionEvent.type ===
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border border-slate-200 dark:border-slate-800 rounded-xl p-4 bg-white dark:bg-slate-950 shadow-xs">
                                            <div className="space-y-2">
                                                <h4 className="text-xs font-bold uppercase text-slate-400 font-mono tracking-wider text-left">
                                                    Session Context
                                                    Initialization
                                                </h4>
                                                <div className="text-xs space-y-2 text-left">
                                                    ...
                                                    <div className="flex justify-between py-1 border-b border-slate-100 dark:border-slate-900">
                                                        ...
                                                        <code className="bg-slate-50 dark:bg-slate-800 px-1 py-0.2 rounded text-slate-700 dark:text-slate-300 font-mono text-[11px]">
                                                            {pWorkingDirectory}
                                                        </code>
                                                    </div>
                                                    <div className="flex justify-between py-1 border-b border-slate-100 dark:border-slate-900">
                                                        ...
                                                        <span className="font-sans font-semibold text-slate-800 dark:text-slate-300">
                                                            {pClientName ||
                                                                "VSCode Ext"}
                                                        </span>
                                                    </div>
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <div className="text-xs space-y-2 text-left">
                                                    <div className="flex justify-between py-1 border-b border-slate-100 dark:border-slate-900">
                                                        ...
                                                        <code className="text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/80 font-mono font-bold px-1.5 py-0.2 rounded">
                                                            {pModel ||
                                                                "gemini-1.5-pro"}
                                                        </code>
                                                    </div>
                                                    <div className="flex justify-between py-1 border-b border-slate-100 dark:border-slate-900">
                                                        ...
                                                        <span className="font-sans font-semibold text-slate-800 dark:text-slate-300 capitalize">
                                                            {pClientMode ||
                                                                "Active (GCP Containers)"}
                                                        </span>
                                                    </div>
                                                    <div className="flex justify-between py-1 border-b border-slate-100 dark:border-slate-900">
                                                        <span className="text-slate-400 font-mono">
                                                ...
                                                binaryItems[0] && (
                                                    <div className="border border-slate-200 dark:border-slate-800 rounded-xl p-3 bg-white dark:bg-slate-900 mt-2 space-y-2 text-left">
                                                        <div className="text-xs font-semibold text-slate-707 dark:text-slate-300 flex items-center gap-1.5">
                                                            ...
                                                            {binaryItems[0]
                                                                .mimeType && (
                                                                <span className="text-[10px] bg-indigo-50 dark:bg-indigo-950/60 text-indigo-650 dark:text-indigo-400 rounded px-1.5 py-0.2">
                                                                    {
                                                                        binaryItems[0]
                                                                            .mimeType
                                                                    }
                                                        ...
                                                        binaryItems[0].data !==
                                                            ...
                                                            <div className="mt-2 text-center rounded-lg border border-slate-200 dark:border-slate-800 p-2 bg-slate-50 dark:bg-slate-950 flex justify-center items-center overflow-auto max-h-[300px]">
                                                                <img
                                                                    ...
                                                                    alt={
                                                                        binaryItems[0]
                                                                            .description ||
                                                                        "Decoded binary telemetry asset"
                                                                    }
                                                            ...
                                                            <div className="relative h-44 bg-slate-950 rounded-lg flex flex-col justify-between p-3 overflow-hidden text-slate-100 select-none">
                                                                <div className="flex justify-between items-center z-10 border-b border-slate-800 pb-1">
                                                                    <span className="text-[10px] font-mono text-slate-404">
                                                                        MEMORY
                                                                        ALLOCATION
                                                                        PROFILE
                                                                        (RSS)
                                                                    </span>
                                                                    <span className="text-[10px] bg-emerald-500/10 text-emerald-400 px-1.5 rounded font-mono font-bold">
                                                                        STABLE
                                                                        AT 124MB
                                                                    </span>
                                                                </div>
                                                                ...
                                                                <div className="absolute inset-0 top-10 flex items-center justify-center opacity-70">
                                                                    <svg
                                                                        className="w-full h-24 stroke-emerald-500 stroke-2 fill-emerald-500/10"
                                                                        viewBox="0 0 100 20"
                                                                    >
                                                                        <path d="M 0 15 Q 15 12, 30 18 T 60 5 T 90 10 L 100 10 L 100 20 L 0 20 Z" />
                                                                        <path
                                                                            d="M 0 15 Q 15 12, 30 18 T 60 5 T 90 10"
                                                                            fill="none"
                                                                            strokeWidth={
                                                                                1
                                                                            }
                                    ...
                                    {event.sessionEvent.type ===
                                        ...
                                        <div className="text-left pl-2 border-l-2 border-rose-500 dark:border-rose-400">
                                            <div className="text-[10px] text-rose-500 font-mono font-bold">
                                                Error:{" "}
                                                {pError || "Session anomaly"}
                                            </div>
                                            <p className="text-[11px] text-slate-600 dark:text-slate-400 font-mono whitespace-pre-wrap">
                                                {pDetails ||
                                    ...
                                    {![
                                        ...
                                        <div className="pl-2 border-l-2 border-slate-200 dark:border-slate-800 space-y-2">
                                            <div className="text-[10px] uppercase text-zinc-500 font-mono tracking-wider font-semibold">
                                                Event Payload
                                            </div>
                                            <div className="text-xs leading-normal font-mono">
                                                ...
                                                {Object.keys(
                                                    ...
                                                    <span className="text-slate-400 italic text-[10px] text-left">
