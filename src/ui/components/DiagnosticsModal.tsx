...
import { CopilotEvent, Scenario, TurnData } from "../mockEvents";
...
interface DiagnosticsModalProps {
    ...
    readonly setScenarioTurns?: (
        scenarioId: string,
        turns: readonly TurnData[],
        events: ReadonlyArray<CopilotEvent>,
    ) => void;
}
...
export function DiagnosticsModal({
    ...
    const [gatesResult, setGatesResult] = useState<GateDiagResult | undefined>(
        undefined,
    );
    ...
    const [cliScriptResult, setCliScriptResult] = useState<
        | {
              pass: boolean;
              durationMs: number;
              output?: string;
        ...
        | undefined
    >(undefined);
    const [cliScriptError, setCliScriptError] = useState<string | undefined>(
        undefined,
    );

    // Run Log Dump State
    const [dumpedLogs, setDumpedLogs] = useState<
        { server: readonly string[]; client: readonly string[] } | undefined
    >(undefined);
    ...
    const [dockerResult, setDockerResult] = useState<
        DockerDiagResult | undefined
    >(undefined);
    const [dockerError, setDockerError] = useState<string | undefined>(
        undefined,
    );
    ...
    const [t5Prompt, setT5Prompt] = useState(
        "Write a quick sum function in typescript and trigger standard checks using npx tsx.",
    );
    ...
    const [t6ReplayTraceId, setT6ReplayTraceId] = useState(
    ...
    const handleCheckGates = async () => {
        ...
        try {
            const res = await fetch(
                "/api/diagnostics/gates?type=ENVIRONMENT_INTEGRITY_CHECK",
            );
    ...
    const runBackendSanityTest = async () => {
        ...
        try {
            const res = await fetch(
                "/api/diagnostics/docker?type=ENVIRONMENT_INTEGRITY_CHECK",
            );
            if (!res.ok) throw new Error(`HTTP error ${res.status}`);
            const data = await res.json();
    ...
    const handleSseSmokeTest = () => {
        ...
        source.onerror = (err) => {
            ...
            setSseRunning(false);
            setSseStatus((prev) =>
                prev === "completed" ? "completed" : "failed",
            );
        };
    };
    ...
    const handleCheckT6 = async () => {
        ...
        try {
            ...
            const traceToUse =
                isReplay && t6ReplayTraceId === "custom"
                    ? customTraceId
                    : t6ReplayTraceId;
            ...
            const streamResponse = await fetch(
                `/api/copilot/gate-stream?sessionId=${currentSessionId}`,
                {
                    method: "GET",
                },
            );
            ...
            if (!streamResponse.ok) {
                throw new Error(
                    `HTTP error ${streamResponse.status}: ${await streamResponse.text()}`,
                );
            }
    ...
    const handleTriggerMainAppReplay = async () => {
        ...
        try {
            const traceToUse =
                t6ReplayTraceId === "custom" ? customTraceId : t6ReplayTraceId;
            ...
            await runWithGates({
                ...
                simulateBackpressureDelayMs:
                    backpressureDelay > 0 ? backpressureDelay : undefined,
                setScenarioTurns,
            });
        } catch (err: unknown) {
    ...
    const handleDumpLogs = async () => {
        ...
        try {
            ...
            const clientLog: readonly string[] = getClientLog
                ? getClientLog()
                : [];
                            ...
                            <div className="flex items-center gap-2.5">
                                ...
                                <div className="text-left">
                                    <h3 className="font-bold text-slate-900 dark:text-white text-base">
                                        Workspace Sandbox Diagnostics Suite
                                    </h3>
                                    <p className="text-xs text-slate-500">
                                        Validate local gate scripts, Docker
                                        environment stability, SSE loops, and
                                        authentic SDK connections.
                                    </p>
                                </div>
                            </div>
                            <button
                                    ...
                                    <div className="space-y-4">
                                        <div className="space-y-1">
                                            <div className="text-[10px] text-slate-500 uppercase tracking-wide font-bold">
                                                Server Subprocess Log buffer
                                                (Latest{" "}
                                                {dumpedLogs.server.length}{" "}
                                                entries)
                                            </div>
                                            <div className="bg-white dark:bg-slate-900 p-2.5 rounded-lg border dark:border-slate-800 max-h-[160px] overflow-y-auto font-mono text-[10px] text-slate-700 dark:text-slate-300">
                                                {dumpedLogs.server.length > 0
                                                    ? dumpedLogs.server.map(
                                                          (l, i) => (
                                                              <div key={i}>
                                                                  {l}
                                                              </div>
                                        ...
                                        <div className="space-y-1">
                                            <div className="text-[10px] text-slate-500 uppercase tracking-wide font-bold">
                                                Client Hook Log buffer (Latest{" "}
                                                {dumpedLogs.client.length}{" "}
                                                entries)
                                            </div>
                                            <div className="bg-white dark:bg-slate-900 p-2.5 rounded-lg border dark:border-slate-800 max-h-[160px] overflow-y-auto font-mono text-[10px] text-slate-700 dark:text-slate-300">
                                                {dumpedLogs.client.length > 0
                                                    ? dumpedLogs.client.map(
                                                          (l, i) => (
                                                              <div key={i}>
                                                                  {l}
                                                              </div>
                                ...
                                {gatesResult && (
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-xs font-mono">
                                        ...
                                        <div className="p-2 bg-white dark:bg-slate-900 rounded-lg border dark:border-slate-800">
                                            ...
                                            <div className="text-[10px] text-slate-500">
                                                Duration:{" "}
                                                {gatesResult.runLint.durationMs}
                                                ms
                                            </div>
                                            <div
                                                className="text-[9px] text-slate-400 mt-1 truncate"
                                                title={
                                                    gatesResult.runLint.output
                                                }
                                            >
                                                Output:{" "}
                                                {gatesResult.runLint.output ||
                            ...
                            <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-slate-100 dark:border-slate-850 space-y-3">
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800 pb-2">
                                    <div className="flex items-center gap-2 font-mono text-xs">
                                        ...
                                        />
                                        <span className="font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                                            T2: Backend Sandbox Environment
                                        </span>
                                    </div>
                                    <button
                                        id="btn-check-docker"
                                ...
                                {dockerResult && (
                                    <div className="space-y-2 text-xs font-mono">
                                        ...
                                        <div className="bg-white dark:bg-slate-900 p-2.5 rounded-lg border dark:border-slate-800">
                                            <div className="text-[10px] text-slate-500 uppercase tracking-wide font-bold mb-1">
                                                Sandbox Environment Probe Output
                                            </div>
                            ...
                            <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-slate-100 dark:border-slate-850 space-y-3">
                                ...
                                <div className="space-y-2 text-xs font-mono">
                                    <div className="flex items-center gap-2 text-[11px]">
                                        ...
                                        {sseStatus === "running" && (
                                            <span className="text-indigo-500 animate-pulse uppercase">
                                                Active Streaming
                                            </span>
                                        )}
                                        {sseStatus === "completed" && (
                                    ...
                                    {sseEvents.length > 0 && (
                                        <div className="bg-white dark:bg-slate-900 p-2.5 rounded-lg border dark:border-slate-800 space-y-1">
                                            <div className="text-[10px] text-slate-500 uppercase tracking-wide font-bold mb-1">
                                                Parsed SSE Buffer Stream
                                            </div>
                                            <div className="max-h-[120px] overflow-y-auto space-y-1 text-[10px] divider-y divider-slate-100">
                                                {sseEvents.map((ev, idx) => (
                                                    <div
                                                        key={idx}
                                                        className="text-slate-600 dark:text-slate-400 flex justify-between"
                                                    >
                                                        <span>➔ {ev.type}</span>
                                                        <span className="text-slate-400 font-light truncate max-w-[250px]">
                                                            {JSON.stringify(ev)}
                                                        </span>
                                                    </div>
                                                ))}
                                ...
                                <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-slate-100 dark:border-slate-850 space-y-3">
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800 pb-2">
                                        <div className="flex items-center gap-2 font-mono text-xs">
                                            ...
                                            />
                                            <span className="font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                                                T6: Diagnostic Gate-Run Sandbox
                                                Test (/api/copilot/gate-run)
                                            </span>
                                        </div>
                                        ...
                                        {!t6Confirm && !t6Running ? (
                                            ...
                                            <div className="flex items-center gap-1.5 font-mono text-[11px]">
                                                <span className="text-indigo-600 dark:text-indigo-400 font-semibold uppercase animate-pulse">
                                                    Execute diagnostic scenario?
                                                </span>
                                                <button
                                                    id="btn-t6-yes"
                                        ...
                                        <div className="space-y-3">
                                            <div className="grid grid-cols-2 gap-2">
                                                <div>
                                                    ...
                                                    >
                                                        <option value="clean_run">
                                                            Clean Run (Pass
                                                            First Time)
                                                        </option>
                                                        <option value="single_retry">
                                                            Single Retry (Fail{" "}
                                                            {"->"} Pass)
                                                        </option>
                                                        <option value="model_escalation">
                                                            Tier Upgrade (Fail
                                                            2x {"->"} Upgrade{" "}
                                                            {"->"} Pass)
                                                        </option>
                                            ...
                                            {t6Scenario === "replay" && (
                                                <div className="space-y-2 p-2 bg-indigo-50/50 dark:bg-indigo-950/10 rounded-lg border border-indigo-100/50 dark:border-indigo-900/30 animate-in fade-in slide-in-from-top-1 duration-200 text-left">
                                                    <div className="grid grid-cols-2 gap-2">
                                                        <div>
                                                            <label className="block text-[9.5px] uppercase font-bold text-indigo-700 dark:text-indigo-400 font-mono mb-1">
                                                                Trace Profile
                                                            </label>
                                                            <select
                                                                value={
                                                            ...
                                                            >
                                                                <option value="token_bucket_v6_trace">
                                                                    Trace: Token
                                                                    Bucket Rate
                                                                    Limiter
                                                                    Validation
                                                    ...
                                                    {t6ReplayTraceId ===
                                                        ...
                                                        <div className="space-y-1 animate-in fade-in duration-150">
                                                            <label className="block text-[9px] uppercase font-bold text-slate-500 font-mono">
                                                                Custom Trace
                                                                Profile ID
                                                            </label>
                                                            <input
                                            ...
                                            <p className="text-[10px] text-slate-400">
                                                This replicates the exact manual
                                                test flow running on
                                                `/api/copilot/gate-run`.
                                                Built-in sandbox gates like lint
                                                and tests will run dynamically.
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    {t6Error && (
                                        <div className="p-2.5 bg-red-50 dark:bg-red-950/20 text-red-600 dark:text-red-400 rounded-lg text-xs font-mono">
                                            Error executing Scenario Stream
                                            test: {t6Error}
                                        </div>
                                    )}
                                                ...
                                                <div className="max-h-[180px] overflow-y-auto space-y-1 text-[9px] text-slate-600 dark:text-slate-400 leading-normal font-mono select-text divide-y divide-slate-100 dark:divide-slate-800">
                                                    {t6Events.map(
                                                        (evt, idx) => (
                                                            <div
                                                                key={idx}
                                                                className="pt-1.5 pb-1 font-mono"
                                                            >
                            ...
                            <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-slate-100 dark:border-slate-850 space-y-3">
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800 pb-2">
                                    ...
                                    {!sdkConfirm && !sdkRunning ? (
                                        ...
                                        <div className="flex items-center gap-1.5 font-mono text-[11px]">
                                            <span className="text-rose-600 font-semibold uppercase animate-pulse">
                                                Are you sure? Will use real
                                                tokens.
                                            </span>
                                            <button
                                                id="btn-test-sdk-yes"
                                    ...
                                    <div className="text-xs font-mono space-y-2">
                                        {sdkAnswer && (
                                            <div className="p-2.5 bg-emerald-50 dark:bg-emerald-950/15 border border-emerald-100 dark:border-emerald-900 text-emerald-800 dark:text-emerald-300 rounded-lg">
                                                <strong className="block uppercase text-[9px] text-emerald-500 font-bold">
                                                    LSP Subprocess Answer
                                                    Response
                                                </strong>
                                        ...
                                        <div className="bg-white dark:bg-slate-900 p-2.5 rounded-lg border dark:border-slate-800">
                                            <div className="text-[10px] text-slate-500 uppercase tracking-wide font-bold mb-1">
                                                Subprocess StdIO Connection Logs
                                            </div>
                            ...
                            <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-slate-100 dark:border-slate-850 space-y-3">
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800 pb-2">
                                    <div className="flex items-center gap-2 font-mono text-xs">
                                        ...
                                        />
                                        <span className="font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                                            T5: Custom Prompt Stream Tester
                                            (/api/copilot/run)
                                        </span>
                                    </div>
                                    ...
                                    {!t5Confirm && !t5Running ? (
                                        ...
                                        <div className="flex items-center gap-1.5 font-mono text-[11px]">
                                            <span className="text-violet-600 font-semibold uppercase animate-pulse">
                                                Run with custom prompt config?
                                            </span>
                                            <button
                                                id="btn-t5-yes"
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <p className="text-[10px] text-slate-400">
                                            Triggers a full simulation. If
                                            sandbox execution tools are
                                            requested, they will be
                                            auto-approved and run in real-time.
                                        </p>
                                    </div>
                                </div>
                                ...
                                {t5Error && (
                                    <div className="p-2.5 bg-red-50 dark:bg-red-950/20 text-red-600 dark:text-red-400 rounded-lg text-xs font-mono">
                                        Error executing Copilot SSE stream test:{" "}
                                        {t5Error}
                                    </div>
                                )}
                            ...
                            <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-slate-100 dark:border-slate-850 space-y-3">
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800 pb-2">
                                    <div className="flex items-center gap-2 font-mono text-xs">
                                        ...
                                        />
                                        <span className="font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                                            T7: Proxy Interception Logs (Gemini
                                            Request Fix)
                                        </span>
                                    </div>
                                    <button
                                ...
                                {proxyLog && (
                                    <div className="space-y-1">
                                        <div className="text-[10px] text-slate-500 uppercase tracking-wide font-bold">
                                            Latest Proxy Activity (Intercepted
                                            requests mapping)
                                        </div>
