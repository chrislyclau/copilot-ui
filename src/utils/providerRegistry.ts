...
export class ProviderRegistry {
    ...
    public getMappedModel(modelName?: string): string {
        if (!modelName) {
            throw new Error(
                `getMappedModel: no model name provided. Set REVIEWER_MODEL (or the relevant *_MODEL env var) to a known model identifier.`,
            );
        }
        ...
        const partialCandidates = MODEL_TIERS.filter(
            (m) => m.includes(cleaned) || cleaned.includes(m),
        );
        ...
        if (
            DEFAULT_ROLES_CONFIG.planner.model === cleaned ||
            DEFAULT_ROLES_CONFIG.planner.model.includes(cleaned)
        ) {
            return DEFAULT_ROLES_CONFIG.planner.model;
        }
        if (
            DEFAULT_ROLES_CONFIG.auditor.model === cleaned ||
            DEFAULT_ROLES_CONFIG.auditor.model.includes(cleaned)
        ) {
            return DEFAULT_ROLES_CONFIG.auditor.model;
        }

        throw new Error(
            `getMappedModel: unknown model "${modelName}". Add it to KNOWN_MODELS_CONFIG in src/config/models.ts or use an exact model identifier.`,
        );
    }
    ...
    public getProviderType(input: string | ModelProviderConfig): ProviderType {
        ...
        if (!matchedConfig) {
            const candidates = allConfigs.filter(
                (t) => model.includes(t.model) || t.model.includes(model),
            );
    ...
    public getProviderConfig(
        ...
        if (provider === "gemini") {
            if (!this.apiKey) {
                throw new Error(
                    "Missing API key for Gemini provider. Expected GEMINI_API_KEY to be set.",
                );
            }
            return {
                type: "openai",
                baseUrl: process.env.COPILOT_API_URL
                    ? `${process.env.COPILOT_API_URL}/api/providers/gemini/v1beta/openai/`
                    : `http://localhost:${process.env.PORT || 3000}/api/providers/gemini/v1beta/openai/`,
                apiKey: this.apiKey,
            };
        } else if (provider === "anthropic") {
            const apiKey =
                process.env.ANTHROPIC_API_KEY ||
                (this.apiKey !== "mock-key" ? this.apiKey : undefined);
            if (!apiKey) {
                throw new Error(
                    "Missing API key for Anthropic provider. Expected ANTHROPIC_API_KEY to be set.",
                );
            }
            return {
                type: "anthropic",
            ...
            if (!apiKey) {
                throw new Error(
                    "Missing API key for OpenRouter provider. Expected OPENROUTER_API_KEY to be set.",
                );
            }

            const proxyBaseUrl = process.env.COPILOT_API_URL
                ? `${process.env.COPILOT_API_URL}/api/providers/openrouter/api/v1/`
                : `http://localhost:${process.env.PORT || 3000}/api/providers/openrouter/api/v1/`;
            ...
            if (!apiKey) {
                throw new Error(
                    "Missing API key for OpenAI provider. Expected OPENAI_API_KEY to be set.",
                );
            }
            return {
                type: "openai",
    ...
    public getExecutionConfig(
        ...
        if (typeof input === "object" && input !== null) {
            ...
            // When the caller already supplies a ModelProviderConfig, use the model
            // name as-is. getMappedModel() is for resolving bare string model names
            // whose provider still needs to be discovered; running it here would
            // silently remap unknown display names (e.g. "Claude Sonnet 4.6 (copilot)")
            // to MODEL_TIERS[0] instead of honouring the caller's intent.
            model = input.model;
        } else {
            ...
            if (matchedConfig) {
                ...
                throw new Error(
                    `getExecutionConfig: could not determine provider for model "${model}". Add it to KNOWN_MODELS_CONFIG in src/config/models.ts.`,
                );
            }
        }
    ...
    static getProviderConfig(
        ...
        return new ProviderRegistry(apiKey).getProviderConfig(
            provider,
            modelName,
        );
    }
