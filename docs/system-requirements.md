# System & Orchestration Requirements (EARS)

Format: [EARS](https://alistairmavin.com/ears/) (Easy Approach to Requirements
Syntax). This document holds the `ORCH-REQ-*` and `SYS-REQ-*` formal
requirements previously inlined in `README.md`; it was split out to keep
`README.md` focused on project orientation. See also `docs/roadmap-spec.md`
(new work, `RM-REQ-*`), `docs/SessionWrapper-spec.md` (current SYS-REQ-028
family), and `docs/orchestration-ears-by-tier.md` (build-status tiering across
all of the above).
